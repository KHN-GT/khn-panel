...
export default function ConvPanel({ item, onApprove, onDiscard, onCorrect }) {
  ...
  const [corrMinimized, setCorrMinimized] = useState(false)
  const [corrExpanded, setCorrExpanded] = useState(false)
  ...
  useEffect(() => {
    ...
    setCorrMode(false); setCorrText(''); setCorrExpanded(false); setIaMinimized(false)
  ...
  const buscarPublicaciones = async (q) => {
    ...
    try {
      ...
      if ((d.publicaciones || []).length > 0 && (textareaRef.current || corrTextareaRef.current)) {
        const activeRef = textareaRef.current || corrTextareaRef.current
  ...
  const insertEmoji = (emoji) => {
    ...
    const cursor = ta?.selectionStart || editText.length
    setEditText(editText.slice(0, cursor) + emoji + editText.slice(cursor))
    setEmojiOpen(false)
    setTimeout(() => { if (ta) { ta.focus(); ta.selectionStart = ta.selectionEnd = cursor + emoji.length } }, 50)
  }
    ...
    <div style={{ display:'flex', flexDirection:'column', height:'100%', overflow:'hidden' }}>
      ...
      <div style={{ padding:'14px 18px', borderBottom:'1.5px solid var(--border)', background:'var(--surface)', flexShrink:0, boxShadow:'var(--shadow)' }}>
        <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:6 }}>
          ...
          {item.tipo === 'PRE-COMPRA' && item.conversion_en && (
            <span style={{ fontSize:12, fontWeight:700, padding:'3px 8px', borderRadius:5,
              background:'#d1fae5', color:'#047857', border:'1px solid #6ee7b7' }}>
              Compra confirmada {item.atendido_en ? (() => {
                const diff = (new Date(item.conversion_en) - new Date(item.atendido_en)) / 3600000
                return diff < 1 ? Math.round(diff * 60) + ' min despues' : Math.round(diff) + 'h despues'
              })() : ''}
            </span>
          )}
          {isResolved && (
        ...
        <div style={{ display:'flex', flexWrap:'wrap', gap:6, marginTop: (item.imagen_thumbnail || item.producto) ? 6 : 0 }}>
          {item.orden_id && (
            <div
              onClick={() => {
                navigator.clipboard.writeText(`https://www.mercadolibre.com.mx/ventas/${item.orden_id}/detalle`)
                  .then(() => { setCopiedLink(true); setTimeout(() => setCopiedLink(false), 1500) })
              }}
              title="Clic para copiar enlace de la orden"
              style={{ fontSize:12, color: copiedLink ? '#22c55e' : 'var(--blue)',
                cursor:'pointer', display:'inline-flex', alignItems:'center', gap:4, transition:'color 0.2s',
                textDecoration: copiedLink ? 'none' : 'underline', textUnderlineOffset:2 }}
            >
              {copiedLink ? '✓ Copiado' : (item.producto || 'Ver orden')}
            </div>
          )}
          {(item.orden_id || (isPostVenta && item.pack_id)) && (
            <div
              onClick={() => {
                const num = item.orden_id || item.pack_id
                navigator.clipboard.writeText(num).then(() => { setCopiedOrden(true); setTimeout(() => setCopiedOrden(false), 1500) })
              }}
              title="Clic para copiar numero de orden"
              style={{ fontSize:11, color: copiedOrden ? '#22c55e' : 'var(--text3)',
                background: copiedOrden ? 'rgba(34,197,94,0.1)' : 'var(--surface2)',
                border: copiedOrden ? '1px solid #22c55e' : '1px solid var(--border)', borderRadius:4, padding:'2px 7px',
            ...
            >
              {copiedOrden ? '✓' : <><code style={{ fontSize:11, fontWeight:600 }}>#{item.orden_id || item.pack_id}</code>
...
<div ref={threadRef} style={{ flex:1, overflowY:'auto', padding:'14px', display:'flex', flexDirection:'column', gap:10 }}>
          ...
          <>
            ...
            {!claimStage && !loadingClaimMsgs && claimMsgs.length === 0 ? (
              ...
              <div style={{ marginTop:12, borderTop:'1px solid var(--border)', paddingTop:10, display:'flex', flexDirection:'column', gap:8 }}>
                <div style={{ padding:'10px 14px', borderRadius:8, background:'#fef3c7', border:'1.5px solid #f59e0b', fontSize:13, color:'#92400e', fontWeight:600, lineHeight:1.5 }}>
                  Este reclamo está en mediación. Responder directamente en MercadoLibre.
                </div>
                <div style={{ display:'flex', gap:8, alignItems:'center' }}>
                  <button disabled style={{
                    flex:1, padding:'8px 0', borderRadius:6, border:'none', fontWeight:700, fontSize:13, cursor:'not-allowed',
                    background:'var(--blue)', color:'#fff', opacity:.4,
                  }}>
                    Enviar a ML
                  </button>
                </div>
              ...
              <div style={{ padding:'12px 14px', borderBottom:'1px solid var(--border)' }}>
                ...
                {(item.orden_id || item.pack_id) && (
                  <div style={{ display:'flex', alignItems:'center', gap:4, marginBottom:8 }}>
                    ...
                    {item.orden_id && (
                      <div
                        onClick={() => window.open(`https://www.mercadolibre.com.mx/ventas/${item.orden_id}/detalle`, '_blank')}
                        title="Abrir orden en MercadoLibre"
                        style={{ fontSize:11, color:'var(--blue)', background:'var(--surface2)',
                          border:'1px solid var(--border)', borderRadius:5, padding:'2px 6px',
                          cursor:'pointer', display:'inline-flex', alignItems:'center', gap:2, transition:'all 0.2s' }}
        ...
        <div style={{ margin:'0 14px 12px', background:'var(--surface)', border:`1.5px solid ${item.respuesta_ia ? 'var(--purple-border)' : 'var(--border)'}`, borderRadius:'var(--radius)', boxShadow:'var(--shadow-md)', overflow:'hidden' }}>
          ...
          {!iaMinimized && (<>{editMode || !item.respuesta_ia
            ? <div style={{ position: 'relative' }}>
                ...
                />
                {/* Emoji picker */}
                <div ref={emojiRef} style={{ position: 'absolute', top: 8, right: 8 }}>
                  <button onClick={() => setEmojiOpen(v => !v)} type="button"
                    style={{ fontSize: 16, background: 'none', border: 'none', cursor: 'pointer', padding: '2px 4px', borderRadius: 4, opacity: 0.6 }}
                    title="Insertar emoji">{'\uD83D\uDE0A'}</button>
                  {emojiOpen && (
                    <div style={{ position: 'absolute', top: 30, right: 0, zIndex: 9999,
                      background: 'var(--surface)', border: '1.5px solid var(--border)',
                      borderRadius: 10, boxShadow: '0 4px 20px rgba(0,0,0,.15)',
                      padding: '8px 10px', width: 210 }}>
                      {[
                        { label: 'Saludos', emojis: ['\uD83D\uDC4B','\uD83D\uDE0A','\uD83D\uDE4F','\u2705','\u2B50'] },
                        { label: 'Positivos', emojis: ['\uD83C\uDF89','\uD83D\uDC4D','\uD83D\uDCAA','\uD83D\uDD25','\u2728'] },
                        { label: 'Soporte', emojis: ['\uD83D\uDCE6','\uD83D\uDE9A','\uD83D\uDD27','\uD83D\uDCCB','\uD83D\uDCA1'] },
                        { label: 'Contacto', emojis: ['\uD83D\uDCF1','\uD83D\uDCF2','\uD83D\uDCAC','\uD83D\uDCDE','\uD83E\uDD1D'] },
                      ].map(cat => (
                        <div key={cat.label} style={{ marginBottom: 6 }}>
                          <div style={{ fontSize: 10, fontWeight: 700, color: 'var(--text3)', textTransform: 'uppercase', marginBottom: 3, letterSpacing: '.04em' }}>{cat.label}</div>
                          <div style={{ display: 'flex', gap: 2 }}>
                            {cat.emojis.map(em => (
                              <button key={em} onClick={() => insertEmoji(em)} type="button"
      ...
      {isResolved && item.tipo === 'PRE-COMPRA' && (
        <div style={{ margin:'0 14px 12px' }}>
          {item.corregido ? (<>
            <button onClick={() => setCorrExpanded(v => !v)}
              style={{ fontSize:12, fontWeight:600, cursor:'pointer', padding:'5px 12px', borderRadius:'var(--radius-sm)',
                background:'#f0fdf4', border:'1px solid #bbf7d0', color:'#16a34a',
                display:'inline-flex', alignItems:'center', gap:4 }}>
              {corrExpanded ? '\u25B2' : '\u25BC'} Ya corregida
            </button>
            {corrExpanded && (
              <div style={{ marginTop:8, padding:'10px 12px', background:'var(--surface)', border:'1px solid var(--border)', borderRadius:'var(--radius-sm)' }}>
                <div style={{ fontSize:11, color:'var(--text2)', marginBottom:6, fontWeight:500, textTransform:'uppercase', letterSpacing:'0.04em' }}>
                  Correccion enviada:
                </div>
                <div style={{ fontSize:13, color:'var(--text)', lineHeight:1.5, marginBottom:10 }}>
                  {item.correccion_humana || item.respuesta_final || '\u2014'}
                </div>
                <button onClick={() => { setCorrExpanded(false); setCorrMode(true); setCorrText(item.correccion_humana || item.respuesta_final || item.respuesta_ia || '') }}
                  style={{ fontSize:12, fontWeight:600, cursor:'pointer', padding:'5px 12px', borderRadius:'var(--radius-sm)',
                    background:'var(--amber-light)', border:'1.5px solid var(--amber-border)', color:'var(--amber)' }}>
                  Corregir de nuevo
                </button>
              </div>
            )}
            ...
            <div style={{ background:'var(--surface)', border:'1.5px solid var(--amber-border)', borderRadius:'var(--radius)', overflow:'hidden' }}>
              <div onClick={() => setCorrMinimized(v => !v)}
                onMouseEnter={e => e.currentTarget.style.filter='brightness(0.95)'}
                onMouseLeave={e => e.currentTarget.style.filter='none'}
                style={{ display:'flex', alignItems:'center', padding:'8px 14px', background:'var(--amber-light)', borderBottom:'1px solid var(--amber-border)', fontSize:13, fontWeight:700, color:'var(--amber)', cursor:'pointer', transition:'filter .15s' }}>
                CORREGIR PARA ENTRENAMIENTO
                <span style={{ marginLeft:'auto', fontSize:14, color:'var(--amber)', lineHeight:1 }}>
                  {corrMinimized ? '▲' : '—'}
                </span>
              </div>
              {!corrMinimized && <><div style={{ position: 'relative' }}>
                <textarea ref={corrTextareaRef} value={corrText} onChange={handleCorrTextareaChange}
                  ...
                  style={{ width:'100%', border:'none', padding:'10px 14px', fontSize:13, fontFamily:'inherit', resize:'vertical', outline:'none', color:'var(--text)', minHeight:120, whiteSpace:'pre-wrap' }} />
