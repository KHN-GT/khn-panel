...
const s = {
  ...
  input: { width:'100%', padding:'10px 12px', border:'1.5px solid var(--border)', borderRadius:'var(--radius-sm)', fontSize:14, color:'var(--text)', background:'var(--surface)', outline:'none', marginBottom:14, transition:'border-color .15s', boxSizing:'border-box' },
...
export default function Login({ onLogin }) {
  ...
  const submit = async e => {
    ...
    try {
      // fetch nativo directo a Railway — no pasa por el proxy de Vite
      ...
      if (!resp.ok || !data.token) {
        setError(data.error || 'Error de acceso — verifica tus datos')
        return
      }
      localStorage.setItem('khn_token', data.token)
      localStorage.setItem('khn_user', JSON.stringify({
        nombre: data.nombre, rol: data.rol, cuentas: data.cuentas,
      }))
      onLogin?.()
    } catch (err) {
