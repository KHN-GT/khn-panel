...
export default function Supervision({ onLogout }) {
  ...
  const [filtUsuarioInput, setFiltUsuarioInput] = useState('')
  const [filtUsuario, setFiltUsuario]           = useState('')
  const filtUsuarioTimer = useRef(null)
  const handleFiltUsuario = useCallback((val) => {
    setFiltUsuarioInput(val)
    clearTimeout(filtUsuarioTimer.current)
    filtUsuarioTimer.current = setTimeout(() => setFiltUsuario(val), 300)
  }, [])
      ...
      <div style={{ padding:'16px 24px', borderBottom:'1.5px solid var(--border)', background:'var(--surface)', flexShrink:0 }}>
        ...
        <div style={{ display:'flex', gap:10, flexWrap:'wrap' }}>
          ...
          <input type='date' value={filtHasta} onChange={e => setFiltHasta(e.target.value)}
            style={{ fontSize:13, padding:'6px 10px', borderRadius:'var(--radius-sm)', border:'1px solid var(--border)', background:'var(--surface)', color:'var(--text)' }} />
          <input type='text' value={filtUsuarioInput} onChange={e => handleFiltUsuario(e.target.value)} placeholder='Buscar por usuario/nickname...'
            style={{ fontSize:13, padding:'6px 10px', borderRadius:'var(--radius-sm)', border:'1px solid var(--border)', background:'var(--surface)', color:'var(--text)', minWidth:180 }} />
          ...
          <button onClick={() => { setFiltCuenta(''); setFiltError(''); setFiltDesde(''); setFiltHasta(''); setFiltRespondido(''); setBusqueda(''); setFiltUsuarioInput(''); setFiltUsuario('') }}
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            {items.filter(item => {
              if (filtUsuario.trim()) {
                ...
                const matchUser = (item.buyer_id || '').toLowerCase().includes(u) ||
                                  (item.agente || '').toLowerCase().includes(u) ||
                                  (item.usuario_respuesta || '').toLowerCase().includes(u)
              ...
              return (
                <div key={item.id} style={{
                  ...
                  {/* Datos del listing */}
                  {(item.producto || item.sku || item.item_id) && (
                        ...
                        <div style={{display:'flex', gap:6, marginTop:2, flexWrap:'wrap'}}>
                          {item.sku && (
                            <span style={{fontSize:10, fontWeight:600, fontFamily:'monospace',
                              ...
                              borderRadius:3, padding:'1px 5px', color:'#6b7280'}}>
                              {item.sku}
                            </span>
                          )}
                          {item.item_id && (
                  ...
                  {item.accion === 'corregido_post_envio' ? (
                    <div>
                      <button
                        ...
                        style={{fontSize:11, fontWeight:500, cursor:'pointer',
                          padding:'3px 10px', borderRadius:4,
                          background:'#f0fdf4', border:'1px solid #bbf7d0',
                          color:'#16a34a', display:'inline-flex', alignItems:'center', gap:4}}>
                        {expandedCorrections[item.id] ? '▲' : '▼'} Ya corregida
                      </button>
                      {expandedCorrections[item.id] && (
                        <div style={{marginTop:8, padding:'10px 12px',
                          ...
                          <button
                            onClick={() => {
                              setCorrModal(item)
                              setCorrTexto(item.correccion_humana || item.respuesta_final || '')
                            }}
                            style={{fontSize:11, fontWeight:500, cursor:'pointer',
                              padding:'3px 10px', borderRadius:4,
