...
export default function Reportes() {
  ...
  const tasaRes   = total ? Math.round(resueltos / total * 100) : 0
  ...
  return (
    <div style={{ height:'100vh', display:'flex', flexDirection:'column', overflow:'hidden' }}>
      ...
      <div style={{ background:'var(--surface)', borderBottom:'1.5px solid var(--border)',
        ...
        <button onClick={() => navigate('/')} style={{ fontSize:18, background:'none',
          border:'none', cursor:'pointer', color:'var(--text3)', padding:'4px 8px', borderRadius:6 }}>←</button>
        ...
        <button onClick={cargar} style={{ marginLeft:'auto', fontSize:13, fontWeight:600, padding:'5px 14px',
          borderRadius:'var(--radius-sm)', background:'var(--purple-light)', color:'var(--purple)',
      ...
      <div style={{ flex:1, overflowY:'auto', padding:'20px 24px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(4, 1fr)', gap:12, marginBottom:20 }}>
          {[
            ...
            { label:'Reclamos', value: reclamos, color:'var(--red)', bg:'var(--red-light)', br:'var(--red-border)' },
            { label:'Tasa resolución', value: tasaRes + '%', color:'var(--amber)', bg:'var(--amber-light)', br:'var(--amber-border)' },
          ].map(m => (
            <div key={m.label} style={{ background:m.bg, border:`1.5px solid ${m.br}`, borderRadius:'var(--radius)',
              padding:'16px 20px' }}>
        ...
        {/* Filtros */}
        <div style={{ background:'var(--surface)', border:'1.5px solid var(--border)', borderRadius:'var(--radius)',
          padding:'14px 16px', marginBottom:16, display:'flex', flexWrap:'wrap', gap:12, alignItems:'center' }}>

          <input value={busqueda} onChange={e => setBusqueda(e.target.value)}
            placeholder="Buscar por comprador, SKU, orden..."
            style={{ fontSize:13, padding:'7px 12px', borderRadius:'var(--radius-sm)',
              border:'1.5px solid var(--border)', color:'var(--text)', flex:1, minWidth:200, outline:'none' }} />
          ...
          <select value={tipo} onChange={e => setTipo(e.target.value)}
            style={{ fontSize:13, padding:'7px 10px', borderRadius:'var(--radius-sm)',
              border:'1.5px solid var(--border)', color:'var(--text)', cursor:'pointer', outline:'none' }}>
          ...
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            ...
            <input type="date" value={fechaDesde} onChange={e => setFechaDesde(e.target.value)}
              style={{ fontSize:12, padding:'6px 8px', borderRadius:'var(--radius-sm)',
        ...
        {loading ? (
          ...
          <div style={{ background:'var(--surface)', border:'1.5px solid var(--border)', borderRadius:'var(--radius)', overflow:'hidden' }}>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 100px 110px 100px 130px 110px',
            ...
            {filtrados.map((item, i) => {
              ...
              return (
                <div key={item.id} style={{ display:'grid', gridTemplateColumns:'1fr 100px 110px 100px 130px 110px',
                  padding:'10px 16px', borderBottom: i < filtrados.length-1 ? '1px solid var(--border)' : 'none',
                  ...
                  <div>
                    <span style={{ fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:5,
                      background:tc.bg, color:tc.color, border:`1px solid ${tc.br}` }}>
                      {item.tipo}
                    </span>
                  </div>
                  <div>
                    <span style={{ fontSize:10, fontWeight:700, padding:'2px 8px', borderRadius:5,
                      background:ec.bg, color:ec.color, border:`1px solid ${ec.br}` }}>
                      {item.estado}
                    </span>
                  </div>
